...
func main() {
    ...
    if err != nil {
        if strings.Contains(err.Error(), "address already in use") {
            log.Printf("El puerto %s ya está en uso. Intentando un puerto aleatorio...", config.Connection.LocalAddress)
            listener, err = minecraft.ListenConfig{
                StatusProvider: p,
            }.Listen("raknet", "")
            if err != nil {
                log.Fatalf("Error al iniciar el proxy: %v", err)
            }
            config.Connection.LocalAddress = listener.Addr().String()
            log.Printf("Proxy iniciado en: %s", config.Connection.LocalAddress)
        } else {
            log.Fatalf("Error al iniciar el proxy: %v", err)
        }
    }
